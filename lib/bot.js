...
async function sendOnce(text, chatId, opts = {}) {
  ...
  try {
    ...
    if (r.ok) { let mid; try { const j = await r.json(); mid = j?.result?.message_id; } catch (_) {} return { ok: true, messageId: mid }; }
...
// Trả message_id (số, truthy) nếu gửi được -> dùng cho luồng 0-conf của track-ws; false nếu hỏng.
async function send(text, chatId = CHAT, opts = {}) {
  console.log(text);
  for (let i = 0; i < 4; i++) {
    const res = await sendOnce(text, chatId, opts);
    if (res.ok) return res.messageId ?? true;
...
// Sửa 1 tin đã gửi (luồng 0-conf -> ✅ đã xác nhận / ⚠️ reorg của track-ws). Trả true nếu OK.
// Bỏ qua lỗi 4xx (vd "message is not modified", message quá cũ) — không thử lại.
async function editMessage(messageId, text, opts = {}, chatId = CHAT) {
  if (!BOT || !chatId || !messageId) return false;
  const body = { chat_id: chatId, message_id: messageId, text, disable_web_page_preview: true };
  if (opts.parseMode) body.parse_mode = opts.parseMode;
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (r.ok) return true;
      if (r.status === 429) { let ra = 1; try { const j = await r.json(); ra = j.parameters?.retry_after || 1; } catch (_) {} await sleep(ra * 1000); continue; }
...
module.exports = { send, editMessage, queueAlert, listen, configured, escapeHtml, _test: { outbox, drain } };
