...
- **Phát hiện launch**: Helius `logsSubscribe` với mentions filter = **co-signer EasyA** (`EASYA_COSIGNER`, override qua `EASYA_WATCH`) — KHÔNG lọc theo config account vì EasyA đã đổi config một lần (2026-08-25: `DD3y…`→`72JM…`) khiến bot lọc-theo-config bỏ lỡ toàn bộ launch; co-signer ký mọi launch và bền qua migration. **commitment BẮT BUỘC `"confirmed"`** (dùng `"processed"` → Helius giao im lặng, mất launch). Lọc log theo marker `InitializeVirtualPoolWith(SplToken|Token2022)`, dedupe theo signature. Websocket chỉ có logs+sig → `getParsedTransaction` (retry) để lấy account. Thứ tự account DBC init: `[2]=dev [3]=mint [5]=pool`; name/symbol/uri từ inner `CreateMetadataAccountV3` (Borsh, `dbc.mjs`).
