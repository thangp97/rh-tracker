...
Khi phát hiện tích hợp, mỗi chu kỳ quét & lưu token index mới, dedupe theo địa chỉ contract.
...
- **Telegram**: mẫu "gửi nhanh, làm giàu bằng edit" — gửi thẻ on-chain ngay (giữ `message_id`), rồi fetch socials qua IPFS (đua gateway, ngoài đường tới hạn) và `editMessageText`.
...
- **Vận hành**: **cần `.env` RIÊNG** trong `easya-tracker/` (Helius key + Telegram token — token Telegram **BẮT BUỘC khác** hai bot kia vì `getUpdates` độc quyền theo token, chung sẽ 409), lockfile single-instance (`easya.lock`), watchdog thoát khi slot ngừng tiến để supervisor restart. Đã thêm app `easya-tracker` vào `ecosystem.config.js`. Test offline (không mạng/secret): `cd easya-tracker && npm test`.
